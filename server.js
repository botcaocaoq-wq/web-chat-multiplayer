...
// Tự động dọn dẹp sạch rác phòng chat sau mỗi 20 phút
setInterval(() => {
    mangTinNhanServer = []; 
...
// Hàm cập nhật và phát số lượng người online cho toàn bộ các máy đang kết nối
function phatTinHieuSoNguoiOnline() {
    // Lấy tổng số lượng thiết bị đang giữ kết nối socket thực tế
...
io.on('connection', (socket) => {
    ...
    phatTinHieuSoNguoiOnline(); // Có người vừa vào web -> Tính lại số người ngay
    ...
    socket.on('disconnect', () => {
        ...
        phatTinHieuSoNguoiOnline(); // Có người thoát web -> Tính toán giảm số người online lập tức
    });
});

const PORT = process.env.PORT || 3000;
http.listen(PORT, () => { console.log(`Server chạy port: ${PORT}`); });
